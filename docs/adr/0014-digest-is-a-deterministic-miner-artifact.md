...
- **Not to be confused with the Proposal index (ADR-0016).** The LLM stage later writes a structured `proposals/YYYY-MM-DD.json` that the Nudge parses for pending-proposal counts. That index is an *LLM-stage* artifact with no deterministic-presence guarantee — it extends the structured-inter-stage-seam pattern, not this ADR's deterministic-presence pattern. The run-happened signal stays the Digest defined here.
